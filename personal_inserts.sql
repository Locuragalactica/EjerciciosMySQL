...
select nombre, comision_emp from empleados group by comision_emp, nombre;
...
select nombre, (sal_emp+comision_emp+500) as Total from empleados where id_depto = 3000 ;
...
select nombre_depto from departamentos  where nombre_depto not in ('ventas', 'Investigacion', 'Mantenimiento')group by nombre_depto;
...
select count(id_depto), nombre_depto from departamentos group by nombre_depto having count(id_depto)>3;
-- 26. Hallar los departamentos que no tienen empleados
select count(id_depto), nombre_depto from departamentos group by nombre_depto having count(id_depto)=0;
...
select * from empleados e inner join departamentos d on e.id_depto = d.id_depto;
...
select id_depto, nombre, sal_emp as 'Salario >= promedio' from empleados where sal_emp >= (select avg(sal_emp) from empleados) order by id_depto;
